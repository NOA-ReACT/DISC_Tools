...
def plot_EC_2D(ax, ds, varname, label,
    ...
    format_plot(ax, ds, title, hmax, dark_mode=dark_mode, timevar=timevar, heightvar=heightvar,
                ...
                plot_position=plot_position, comparison=comparison, yticks=yticks, scc=scc, 
    ...
    if total_backscatter:
        ...
        if comparison:
            ...
            plot_EC_2D(
                ...
                scc=False, yticks=True, smoother=smoother, 
...
def quicklook_AEBD(AEBD, resolution='high', hmax=30e3, dstdir=None, axes=None, 
                   comparison=False, station=None, show_temperature=False, 
                   with_marble=False):
    ...
    if axes is None:
        if comparison:
            nrows = 4  # Comparison mode uses 4 plots
        else:
            nrows = 5  # Standard mode uses 5 plots (includes classification)
        fig, axes = plt.subplots(figsize=(25, 7*nrows), nrows=nrows,
                                 gridspec_kw={'hspace': 0.75})
    else:
        fig = None  # No new figure created
        expected_axes = 4 if comparison else 5
        if len(axes) < expected_axes:
            raise ValueError(f"Provide {expected_axes} axes for AEBD plotting")

    if comparison:
        # Comparison mode - 4 plots without classification
        # Particle backscatter
        plot_EC_2D(
            axes[0], AEBD, 'particle_backscatter_coefficient_355nm' + suffix,
            r"$\beta$", cmap=colormaps.calipso_smooth,
            plot_scale='log', title=f'ATL-EBD particle backscatter - {resolution} res.',
        ...
        plot_EC_2D(
            ...
            plot_scale='linear', title='ATL-EBD linear depolarization ratio',
            plot_range=[0, 0.5],  # Using Git version's range
            units='-', hmax=hmax,
            plot_position='middle', station=station,
        ...
        # Standard plotting without comparison - 5 plots including classification
        
        # Simple classification (from Git version)
        ...
        # Particle backscatter (using Git version's variable name)
        plot_EC_2D(
            axes[1], AEBD, 'particle_backscatter_coefficient_355nm' + suffix,
        ...
        # Depolarization ratio
        plot_EC_2D(
            axes[2], AEBD, f'particle_linear_depol_ratio_355nm{suffix}',
            r"$\delta$", cmap=colormaps.chiljet2,
        ...
        # Particle extinction
        plot_EC_2D(
            axes[3], AEBD, 'particle_extinction_coefficient_355nm' + suffix,
    ...
    # Add temperature contours if requested
    if show_temperature:
        # Add temperature contours if temperature variable exists
        if 'temperature' in AEBD.data_vars:
            for ax in axes:
                add_temperature(ax, AEBD)
        
        # Add tropopause line if tropopause_height exists
        if 'tropopause_height' in AEBD.data_vars:
            for ax in axes:
                add_boundary_line(ax, AEBD, 'tropopause_height', 
                                 color='black', linewidth=2, label='Tropopause')
    
    # Add subfigure labels (from Git version)
    if fig is not None:
        add_subfigure_labels(axes)
    ...
    if dstdir and fig is not None:
        ...
        # Return both fig and axes like in Git version when no dstdir
        return fig, axes
    else:
        # When using external axes, return None
        return None
